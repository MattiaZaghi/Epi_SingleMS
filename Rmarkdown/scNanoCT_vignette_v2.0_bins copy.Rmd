...
knitr::opts_knit$set(root.dir = "/date/gcb/gcb_wq/nanoCTAR_pipeline_barcode_correction")
```
```{r setup2}
rm(list = ls())
library(Signac)
...
source(paste0(repodir,"scripts/GetCells.R"))
```
...
wd <- "/date/gcb/gcb_wq/nanoCTAR_pipeline_barcode_correction" # change accordingly
setwd(wd)
```
...
for (smpl in samples) {
  ...
  for (mod in modalities) {
    ...
    #metadata.ls[[paste0(mod,"_",smpl)]] <-  metadata.ls[[paste0(mod,"_",smpl)]] %>% dplyr::mutate(passedMZ=logUMI>1.5 & peak_ratio_MB>0.3)
 }
}
...
#plotPassed(metadata.ls,xaxis_text = 9,angle_x=60)
 metadata.ls <- gateCells(metadata.ls, ref_col = "passedManual")
```
...
```{r filter_out_cells}
metadata.ls <- lapply(metadata.ls, function(x){x[x$passedManual,]})
```
