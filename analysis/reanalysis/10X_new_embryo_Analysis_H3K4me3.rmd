...
# Samples
#samples <- dir()
samples<-c("nanoCTAR_e13_K4me3_1","nanoCTAR_e13_K4me3_2")
# Modalities
#modalities <- dir(samples[1]) # this assumes sample1, sample2, sampleN share the same modalities and barcodes!
modalities<-c("ATAC_TATAGCCT","H3K4me3_CCTATCCT","H3K27me3_ATAGAGGC")
...
for (smpl in samples) {
  ...
  for (mod in modalities) {
    ...
    metadata.ls[[paste0(mod,"_",smpl)]] <-  metadata.ls[[paste0(mod,"_",smpl)]] %>% dplyr::mutate(passedMZ=logUMI>2.5)
 }
}
...
for (experim in names(fragment.ls)) {
  ...
  counts.ls[[experim]] <- GenomeBinMatrix(fragments = fragment.ls[[experim]],         # fragments
                                        ...
                                        process_n = 2000000) # the higher, the faster, the more memory is used
...
# First step we have to extract all the barcodes that common across 4 modalities
    ...
    x <- list(name_mod1=colnames(combined.obj.ls[["ATAC_TATAGCCT"]]),
              name_mod2=colnames(combined.obj.ls[["H3K4me3_CCTATCCT"]]),
...
# Identify the cell barcodes that you want to keep
 combined.obj.ls_unique<-list()
barcodes_to_keep <- barcode_list
for (mod in modalities){
# Use the subset function to filter the Seurat object
 combined.obj.ls_unique[[mod]] <- subset(combined.obj.ls[[paste0(mod)]], cells = barcodes_to_keep)
...
# Normalisation
combined.obj.ls_unique <- lapply(combined.obj.ls_unique,RunTFIDF)
...
# Variable features
combined.obj.ls_unique <- lapply(combined.obj.ls_unique,FindTopFeatures,min.cutoff = 5)

# Dim reduction
combined.obj.ls_unique <- lapply(combined.obj.ls_unique,RunSVD)
```
...
plot.list <- lapply(combined.obj.ls_unique,DepthCorMulMod)
...
```{r fig.width = 10, fig.height = 4}
plot.list_elbow <- lapply(combined.obj.ls_unique,ElbowPlot,reduction = "lsi",ndims = 50)
...
# ATAC
combined.obj.ls_unique$ATAC_TATAGCCT <- RunUMAP(combined.obj.ls_unique$ATAC_TATAGCCT,reduction = 'lsi', dims = 2:pc)
combined.obj.ls_unique$ATAC_TATAGCCT <- FindNeighbors(combined.obj.ls_unique$ATAC_TATAGCCT,reduction = 'lsi', dims = 2:pc)
combined.obj.ls_unique$ATAC_TATAGCCT <- FindClusters(combined.obj.ls_unique$ATAC_TATAGCCT, verbose = FALSE, algorithm = 3,resolution = .2)
# algorithm = 3 means SLM algorithm is used for modularity optimization

# H3K4me3
combined.obj.ls_unique$H3K4me3_CCTATCCT <- RunUMAP(combined.obj.ls_unique$H3K4me3_CCTATCCT,reduction = 'lsi', dims = 2:pc)
combined.obj.ls_unique$H3K4me3_CCTATCCT <- FindNeighbors(combined.obj.ls_unique$H3K4me3_CCTATCCT,reduction = 'lsi', dims = 2:pc)
combined.obj.ls_unique$H3K4me3_CCTATCCT <- FindClusters(combined.obj.ls_unique$H3K4me3_CCTATCCT, verbose = FALSE, algorithm = 3,resolution = .2)

# H3K27me3
combined.obj.ls_unique$H3K27me3_ATAGAGGC <- RunUMAP(combined.obj.ls_unique$H3K27me3_ATAGAGGC,reduction = 'lsi', dims = 2:pc)
combined.obj.ls_unique$H3K27me3_ATAGAGGC <- FindNeighbors(combined.obj.ls_unique$H3K27me3_ATAGAGGC,reduction = 'lsi', dims = 2:pc)
combined.obj.ls_unique$H3K27me3_ATAGAGGC <- FindClusters(combined.obj.ls_unique$H3K27me3_ATAGAGGC, verbose = FALSE, algorithm = 3,resolution = .2)
```

```{r fig.width = 10, fig.height = 4}
# Plot each modality separately
p1=DimPlot(combined.obj.ls_unique$ATAC_TATAGCCT, label = TRUE) + NoLegend() +
  ggtitle("ATAC") +
  theme(plot.title = element_text(size=15,hjust=0.5,face='bold'))

p2=DimPlot(combined.obj.ls_unique$H3K4me3_CCTATCCT, label = TRUE) + NoLegend() +
  ggtitle("H3K4me3") +
  theme(plot.title = element_text(size=15,hjust=0.5,face='bold'))

p3=DimPlot(combined.obj.ls_unique$H3K27me3_ATAGAGGC, label = TRUE) + NoLegend() +
  ggtitle("H3K27me3") +
  theme(plot.title = element_text(size=15,hjust=0.5,face='bold'))
...
```{r}
saveRDS(obj.ls.qc,"/proj/user/mattia/Embryo_2/obj.ls.qc_nanoCTAR_e13_K4_5kb_bins.rds")
saveRDS(combined.obj.ls,"/proj/user/mattia/Embryo_2/combined.obj.ls_epi_mod_nanoCTAR_e13_K4_5kb_bins.rds")
