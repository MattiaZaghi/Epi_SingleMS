...
require("knitr")
opts_knit$set(root.dir = "/date/gcb/gcb_MZ/multiNanoCT/samples/Human_PBMCs_scCut&Tag_Pro/")# change accordingly
...
modalities<-c("H3K4me3_ATAGAGGC","H3K27me3_TATAGCCT")# this assumes sample1, sample2, sampleN share the same modalities and barcodes!
...
for (smpl in samples) {
  ...
  for (mod in modalities) {
    ...
    # Load metadata
    metadata.ls[[paste0(mod,"_",smpl)]] <- read.csv(paste0("/date/gcb/gcb_MZ/multiNanoCT/samples/Human_PBMCs_scCutTag_Pro",smpl,"/",mod,"/cell_picking/metadata.csv"),
                                                  stringsAsFactors = FALSE)
    rownames(metadata.ls[[paste0(mod,"_",smpl)]]) <- metadata.ls[[paste0(mod,"_",smpl)]]$barcode
    
    metadata.ls[[paste0(mod,"_",smpl)]] <-  metadata.ls[[paste0(mod,"_",smpl)]] %>% dplyr::mutate(passedMZ=logUMI>3 & passedMB=="TRUE")
 }
}
...
for (experim in names(fragment.ls)) {
  ...
  counts_bins.ls[[experim]] <- GenomeBinMatrix(fragments = fragment.ls[[experim]],         # fragments
                                        ...
                                        process_n = 10000000) # the higher, the faster, the more memory is used
...
for (mod in modalities) {
  ...
  combined.obj.ls_epi_mod[[mod]] <- merge(obj.ls.qc[[paste0(mod,"_",samples[1])]],
                                  y = c(obj.ls.qc[[paste0(mod,"_",samples[2])]]),
...
combined.obj.ls_epi_mod[[mod]] <- subset(combined.obj.ls_epi_mod[[paste0(mod)]], cells = barcodes_to_keep)
...
# Normalisation
combined.obj.ls_scCUTTAG_Pro<- lapply(combined.obj.ls_scCUTTAG_Pro,RunTFIDF)
...
# Variable features
combined.obj.ls_scCUTTAG_Pro<- lapply(combined.obj.ls_scCUTTAG_Pro,FindTopFeatures,min.cutoff = 5)

# Dim reduction
combined.obj.ls_scCUTTAG_Pro <- lapply(combined.obj.ls_scCUTTAG_Pro,RunSVD)
```
...
plot.list <- lapply(combined.obj.ls_scCUTTAG_Pro,DepthCorMulMod)
...
```{r fig.width = 10, fig.height = 4}
plot.list_elbow <- lapply(combined.obj.ls_scCUTTAG_Pro,ElbowPlot,reduction = "lsi",ndims = 50)
...
# ATAC
combined.obj.ls_scCUTTAG_Pro$H3K27ac_CCTATCCT <- RunUMAP(combined.obj.ls_scCUTTAG_Pro$H3K27ac_CCTATCCT,reduction = 'lsi', dims = 2:pc)
combined.obj.ls_scCUTTAG_Pro$H3K27ac_CCTATCCT <- FindNeighbors(combined.obj.ls_scCUTTAG_Pro$H3K27ac_CCTATCCT,reduction = 'lsi', dims = 2:pc)
combined.obj.ls_scCUTTAG_Pro$H3K27ac_CCTATCCT <- FindClusters(combined.obj.ls_scCUTTAG_Pro$H3K27ac_CCTATCCT, verbose = FALSE, algorithm = 3,resolution = .2)
# algorithm = 3 means SLM algorithm is used for modularity optimization

# H3K27ac
combined.obj.ls_scCUTTAG_Pro$H3K4me3_ATAGAGGC_PBMCs <- RunUMAP(combined.obj.ls_scCUTTAG_Pro$H3K4me3_ATAGAGGC_PBMCs ,reduction = 'lsi', dims = 2:pc)
combined.obj.ls_scCUTTAG_Pro$H3K4me3_ATAGAGGC_PBMCs <- FindNeighbors(combined.obj.ls_scCUTTAG_Pro$H3K4me3_ATAGAGGC_PBMCs,reduction = 'lsi', dims = 2:pc)
combined.obj.ls_scCUTTAG_Pro$H3K4me3_ATAGAGGC_PBMCs<- FindClusters(combined.obj.ls_scCUTTAG_Pro$H3K4me3_ATAGAGGC_PBMCs, verbose = FALSE, algorithm = 3,resolution = .2)

# H3K27me3
combined.obj.ls_scCUTTAG_Pro$H3K27me3_TATAGCCT_PBMCs <- RunUMAP(combined.obj.ls_scCUTTAG_Pro$H3K27me3_TATAGCCT_PBMCs,reduction = 'lsi', dims = 2:pc)
combined.obj.ls_scCUTTAG_Pro$H3K27me3_TATAGCCT_PBMCs <- FindNeighbors(combined.obj.ls_scCUTTAG_Pro$H3K27me3_TATAGCCT_PBMCs,reduction = 'lsi', dims = 2:pc)
combined.obj.ls_scCUTTAG_Pro$H3K27me3_TATAGCCT_PBMCs <- FindClusters(combined.obj.ls_scCUTTAG_Pro$H3K27me3_TATAGCCT_PBMCs, verbose = FALSE, algorithm = 3,resolution = .2)
```


```{r fig.width = 10, fig.height = 4}
# Plot each modality separately
p1=DimPlot(combined.obj.ls_scCUTTAG_Pro$H3K27ac_CCTATCCT, label = TRUE) + NoLegend() +
  ggtitle("H3K27ac") +
  theme(plot.title = element_text(size=15,hjust=0.5,face='bold'))

p2=DimPlot(combined.obj.ls_scCUTTAG_Pro$H3K4me3_ATAGAGGC_PBMCs, label = TRUE) + NoLegend() +
  ggtitle("H3K4me3") +
  theme(plot.title = element_text(size=15,hjust=0.5,face='bold'))

p3=DimPlot(combined.obj.ls_scCUTTAG_Pro$H3K27me3_TATAGCCT_PBMCs, label = TRUE) + NoLegend() +
  ggtitle("H3K27me3") +
  theme(plot.title = element_text(size=15,hjust=0.5,face='bold'))
...
```{r}
saveRDS(obj.ls.qc,"/date/gcb/gcb_MZ/multiNanoCT/samples/Analysis/obj.ls.qc_scCut&Tag_pro_H3K4me3_27me3_5kb_bins.rds")
saveRDS(combined.obj.ls_epi_mod,"/date/gcb/gcb_MZ/multiNanoCT/samples/Analysis/combined.obj.ls_epi_scCut&Tag_pro_H3K4me3_27me3_5kb_bins.rds")
```
