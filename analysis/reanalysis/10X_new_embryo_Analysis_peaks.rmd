...
# Samples
#samples <- dir()
samples<-c("nanoCTAR_e13_1","nanoCTAR_e13_2")
# Modalities
...
for (smpl in samples) {
  ...
  for (mod in modalities) {
    ...
    if (mod2 != "ATAC") {
      cat("\t(macs peaks)\n")
      input.ls[[paste0(mod,"_",smpl)]] <- read.table(paste0("/proj/user/mattia/Embryo_2/",smpl,"/",mod,"/peaks/macs_broad/",mod2,"_peaks.broadPeak"))[1:3]
      colnames(input.ls[[paste0(mod,"_",smpl)]] ) <- c("chr", "start", "end")
    } else {
      cat("\t(cellranger peaks)\n")
      input.ls[[paste0(mod,"_",smpl)]] <- read.table(paste0("/proj/user/mattia/Embryo_2/",smpl,"/",mod,"/cellranger/outs/peaks.bed"))[1:3]
      colnames(input.ls[[paste0(mod,"_",smpl)]] ) <- c("chr", "start", "end")
    }
...
for (experim in names(fragment.ls)) {
  ...
  counts.ls[[experim]] <- FeatureMatrix(fragments = fragment.ls[[experim]],           # fragments
                                        ...
                                        process_n = 200000) # the higher, the faster, the more memory is used
...
# H3K27ac
combined.obj.ls$H3K27ac_CCTATCCT <- RunUMAP(combined.obj.ls$H3K27ac_CCTATCCT,reduction = 'lsi', dims = 2:40)
combined.obj.ls$H3K27ac_CCTATCCT <- FindNeighbors(combined.obj.ls$H3K27ac_CCTATCCT,reduction = 'lsi', dims = 2:40)
combined.obj.ls$H3K27ac_CCTATCCT <- FindClusters(combined.obj.ls$H3K27ac_CCTATCCT, verbose = FALSE, algorithm = 3,resolution = .6)

# H3K27me3
combined.obj.ls$H3K27me3_ATAGAGGC <- RunUMAP(combined.obj.ls$H3K27me3_ATAGAGGC,reduction = 'lsi', dims = 2:40)
combined.obj.ls$H3K27me3_ATAGAGGC <- FindNeighbors(combined.obj.ls$H3K27me3_ATAGAGGC,reduction = 'lsi', dims = 2:40)
...
p2=DimPlot(combined.obj.ls$H3K27ac_CCTATCCT, label = TRUE) + NoLegend() +
  ggtitle("H3K27ac") +
  theme(plot.title = element_text(size=15,hjust=0.5,face='bold'))
...
combined.obj.ls$ATAC_TATAGCCT <- subset(combined.obj.ls$ATAC_TATAGCCT,nCount_peaks>100 )
combined.obj.ls$H3K27ac_CCTATCCT <- RunHarmony(
  combined.obj.ls$H3K27ac_CCTATCCT,
  group.by.vars = c("orig.ident"),
  reduction.use = "lsi",
  reduction.save = "harmony_lsi",
  project.dim = FALSE
)
combined.obj.ls$H3K27ac_CCTATCCT <- FindNeighbors(combined.obj.ls$H3K27ac_CCTATCCT, reduction = "harmony_lsi", dims = 1:40,k.param = 20 ,nn.method = "annoy", n.trees = 50, annoy.metric = "euclidean",graph.name=c("harmony_nn","harmony_snn"))
combined.obj.ls$H3K27ac_CCTATCCT <- FindClusters(combined.obj.ls$H3K27ac_CCTATCCT, resolution = 0.5,graph.name ="harmony_snn", cluster.name="clusters_harmony_lsi")
combined.obj.ls$H3K27ac_CCTATCCT<- RunUMAP(combined.obj.ls$H3K27ac_CCTATCCT, reduction = "harmony_lsi",dims = 1:40,reduction.name = "UMAP_harmony_lsi")

p1<-DimPlot(combined.obj.ls$H3K27ac_CCTATCCT, reduction = "UMAP_harmony_lsi",group.by = "clusters_harmony_lsi")
p1
```
