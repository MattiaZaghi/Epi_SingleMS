...
require("knitr")
opts_knit$set(root.dir = "/date/gcb/gcb_MZ/nanoCTAR_YD/mouse/")# change accordingly
...
# Samples
#samples <- dir()
samples<-c("embryo_sorted","embryo_unsorted")
# Modalities
...
for (smpl in samples) {
  ...
  for (mod in modalities) {
    ...
    # Load metadata
    metadata.ls[[paste0(mod,"_",smpl)]] <- read.csv(paste0("/date/gcb/gcb_MZ/nanoCTAR_YD/mouse/",smpl,"/",mod,"/cell_picking/metadata.csv"),
                                                  stringsAsFactors = FALSE)
    rownames(metadata.ls[[paste0(mod,"_",smpl)]]) <- metadata.ls[[paste0(mod,"_",smpl)]]$barcode
    metadata.ls[[paste0(mod,"_",smpl)]] <-  metadata.ls[[paste0(mod,"_",smpl)]] %>% dplyr::mutate(passedMZ_H3K4me3_CCTATCCT=logUMI>2.15)
    metadata.ls[[paste0(mod,"_",smpl)]] <-  metadata.ls[[paste0(mod,"_",smpl)]] %>% dplyr::mutate(passedMZ_H3K27me3_ATAGAGGC=logUMI>2.69)
 }
}
...
```{r filter_out_cells}
# Define your modality list
modalities <- c("H3K4me3_CCTATCCT","H3K27me3_ATAGAGGC")  # adjust to your actual modalities

# Store original names
original_names <- names(metadata.ls)

# Filter each dataframe by its modality-specific cutoff
metadata.ls <- lapply(names(metadata.ls), function(nm) {
  # Extract the modality for this element
  mod <- NULL
  for (m in modalities) {
    if (grepl(m, nm)) {
      mod <- m
  ...
  # Construct the modality-specific cutoff column name
  if (!is.null(mod)) {
  ...
  # Filter rows where the cutoff column is TRUE
...
for (smpl in samples) {
  ...
  for (mod in modalities) {
    ...
    fragment.ls[[paste0(mod,"_",smpl)]] <- CreateFragmentObject(
                                              path = paste0("/date/gcb/gcb_MZ/nanoCTAR_YD/mouse/",smpl,"/",mod,"/cellranger/outs/fragments_raw.tsv.gz"),
...
Since we do not have replicates here i just change the name of the object to keep it consistent with the rest of the code
...
# Variable features
combined.obj.ls <- lapply(combined.obj.ls,FindTopFeatures,min.cutoff = 5)
...
ggarrange(plot.list_elbow[[1]],plot.list_elbow[[1]],ncol=2)
#
pc <- 10
```
...
p1=DimPlot(combined.obj.ls$H3K4me3_CCTATCCT, label = TRUE) + NoLegend() +
  ggtitle("H3K4me3") +
  theme(plot.title = element_text(size=15,hjust=0.5,face='bold'))

p2=DimPlot(combined.obj.ls$H3K27me3_ATAGAGGC, label = TRUE) + NoLegend() +
  ggtitle("H3K27me3") +
  theme(plot.title = element_text(size=15,hjust=0.5,face='bold'))
...
saveRDS(combined.obj.ls,"/date/gcb/gcb_MZ/nanoCTAR_YD/mouse/combined.obj.ls")#if you want to redo the analysis yourself you can also load this object with readRDS
saveRDS(obj.ls.qc,"/date/gcb/gcb_MZ/nanoCTAR_YD/mouse/obj.ls.qc")#if you want to redo the analysis yourself you can also load this object with readRDS
```
\
...
combined.obj.ls$H3K4me3_CCTATCCT[['RNA']] <- CreateAssayObject(counts = gene.activities)
