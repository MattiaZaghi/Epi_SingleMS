...
plotPassed <- function(mdata.list, modalities, xaxis_text = NULL, angle_x = NULL) {
  if (is.null(xaxis_text)) { xaxis_text = 12 }
  if (is.null(angle_x)) { angle_x = 30 }
  ...
  for (exp in names(mdata.list)) {
    ...
    # Identify modality from the name
    mod <- NULL
    for (m in modalities) {
      if (grepl(m, exp)) {
        mod <- m
    ...
    # Determine which cutoff column to use
    if (!is.null(mod)) {
      cutoff_col <- paste0("passedMZ_", mod)
    } else {
      cutoff_col <- "passedMB"  # fallback
    }
    
    # Check if the cutoff column exists, otherwise use passedMB
    if (cutoff_col %in% colnames(df)) {
      counts <- as.data.frame(table(df[[cutoff_col]]))
    } else {
      warning(paste("Column", cutoff_col, "not found in", exp, "- using passedMB"))
      counts <- as.data.frame(table(df$passedMB))
      cutoff_col <- "passedMB"
    }
    ...
    counts$Perc <- counts$Freq / sum(counts$Freq) * 100
    counts$sample <- exp
    counts$modality <- mod  # Store modality for reference
    
    # Rename the first column to a generic name for plotting
    names(counts)[1] <- "passed_cutoff"
  ...
  # Plot
  pl <- ggplot(metadata.df, aes(x = sample, y = Freq, fill = passed_cutoff)) +
    theme_bw() +
    geom_bar(stat = "identity", color = 'black', alpha = .7) +
    # x-axis
    theme(axis.text.x = element_text(angle = angle_x, hjust = 1, size = xaxis_text)) + 
...
plotPassedCells <- function(obj, sample_list, mod_list, cutoff_map = NULL) {
  # If cutoff_map is provided, use it; otherwise auto-generate column names based on modality
  ...
  for (nm in names(obj)) {
    ...
    # Get the appropriate cutoff column for this modality
    if (!is.null(cutoff_map) && mod %in% names(cutoff_map)) {
      # Use the cutoff map if provided
      cutoff_col <- cutoff_map[[mod]]
    } else {
      # Auto-generate: construct column name like "passedMZ_K4", "passedMZ_K27", etc.
      cutoff_col <- paste0("passedMZ_", mod)
    }
    
    # Assign the modality-specific cutoff to a common column name
    if (cutoff_col %in% colnames(obj[[nm]])) {
      obj[[nm]]$passed_cutoff <- obj[[nm]][[cutoff_col]]
    } else {
      warning(paste("Column", cutoff_col, "not found for", nm, "- checking for default 'passedMB'"))
      # Fallback to passedMB if the modality-specific column doesn't exist
      if ("passedMB" %in% colnames(obj[[nm]])) {
        obj[[nm]]$passed_cutoff <- obj[[nm]][["passedMB"]]
      } else {
        obj[[nm]]$passed_cutoff <- NA
      }
    }
  ...
  plot <- ggplot(df, aes(x = all_unique_MB, y = peak_ratio_MB, fill = passed_cutoff)) + 
    theme_bw() +
